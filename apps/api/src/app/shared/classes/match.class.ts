...
export class Match implements MatchInterface {
    ...
    play(from: BoardPosition, to: BoardPosition) {
        ...
        if (pieceToMove.getColor() !== this.turn) throw new HttpException(`Peace @${x0},${y0} is not ${this.turn}`, HttpStatus.FORBIDDEN);
        ...
        if (squareToOccupy && squareToOccupy.getColor() === this.turn) throw new HttpException(`Peace @${x1},${y1} is also ${this.turn}`, HttpStatus.FORBIDDEN);
